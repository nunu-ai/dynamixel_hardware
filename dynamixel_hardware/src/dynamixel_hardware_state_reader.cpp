...
{
constexpr const char * kDynamixelHardware = "DynamixelHardwareStateReader";
...
return_type DynamixelHardwareStateReader::configure(const hardware_interface::HardwareInfo & info)
{
...
DynamixelHardwareStateReader::export_state_interfaces()
{
...
DynamixelHardwareStateReader::export_command_interfaces()
{
...
return_type DynamixelHardwareStateReader::start()
{
...
return_type DynamixelHardwareStateReader::stop()
{
...
return_type DynamixelHardwareStateReader::read()
{
  if (use_dummy_) {
    return return_type::OK;
  }
...
return_type DynamixelHardwareStateReader::write()
{
...
return_type DynamixelHardwareStateReader::enable_torque(const bool enabled)
{
  const char * log = nullptr;
...
return_type DynamixelHardwareStateReader::set_control_mode(
...
return_type DynamixelHardwareStateReader::reset_command()
{
  for (uint i = 0; i < joints_.size(); i++) {
...
PLUGINLIB_EXPORT_CLASS(
  dynamixel_hardware::DynamixelHardwareStateReader, hardware_interface::SystemInterface)
